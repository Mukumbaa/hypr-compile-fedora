...
Epoch:          1
Version:        %{?module_version}%{!?module_version:1.4.1}
...
%prep
%autosetup -c -p1
# Se è presente un sotto-progetto sdbus-cpp da estrarre
if [ -d "subprojects/sdbus-cpp" ] && [ -f "%{SOURCE1}" ]; then
  tar -xf %{SOURCE1} -C subprojects/sdbus-cpp --strip=1 || true
fi
...
%changelog
* Mon Sep 21 2026 builder <builder@localhost> - %{epoch}:%{version}-%{release}
- Native Build
