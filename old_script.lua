...
if ver_choice == "2" then
  checkout_cmd = [[
    echo "-> Searching last relese..."
    LATEST_TAG=$(git tag -l | sort -V | tail -n 1)
    echo "-> Release found: $LATEST_TAG"
    git checkout $LATEST_TAG 2>/dev/null || echo "Error in checkout, staying on HEAD"
    git submodule update --init --recursive
  ]]
  print("-> Target: Last release")
else
  checkout_cmd = [[
    echo "-> Last git commit (master/main)"
    git submodule update --init --recursive
  ]]
...
run([[
    if [ ! -d "/usr/include/glaze" ]; then
        ...
        sudo cp -r /tmp/glaze/include/glaze /usr/include/
        rm -rf /tmp/glaze
        echo "Glaze installated in /usr/include/glaze"
    else
...
Version:        %s
Release:        1%%{?dist}
Summary:        %s module built from source
License:        GPL/MIT
Provides:       %s
Conflicts:      %s
AutoReqProv:    yes
...
EOF
    ...
    file_list,
    spec_path,
    rpm_name, module_version, rpm_name, rpm_name, rpm_name,
    rpm_name,
    file_list,
